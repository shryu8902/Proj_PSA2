...
for file in tqdm.tqdm(TEST_files):
    ...
    save_root = 'D:/ie_201008/all/'
    filename = os.path.basename(file).replace('.csv','')
    np.save(save_root+filename, temp2)
with open('D:/ie_201008/SCALERS.pickle','wb') as f:
    pickle.dump(SCALERS,f)

#%%
TRAIN_input =np.load('D:/ie_201008/train/train_input.npy')
TEST_input = np.load('D:/ie_201008/test/test_input.npy')
TRAIN_list = np.load('D:/ie_201008/train/train_list.npy')
TEST_list = np.load('D:/ie_201008/test/test_list.npy')

ALL_input = pd.DataFrame(np.concatenate([TRAIN_input,TEST_input]))
ALL_input = All_input.sort_values([0], ascending = True)
np.save('D:/ie_201008/ie_input.npy')
...
TRAIN_list = np.load('D:/ie_201008/train/train_list.npy')
TRAIN_input =np.load('D:/ie_201008/train/train_input.npy')
TRAIN_index1 = sorted([int(os.path.basename(x).split('.csv')[0]) for x in TRAIN_files])
TRAIN_index2 = sorted(TRAIN_list)

TRAIN_index3 = sorted([int(x) for x in TRAIN_input[:,0]])
TRAIN_index1==TRAIN_index2
TRAIN_index3==TRAIN_index2
len(TRAIN_index3)
type(TRAIN_index2)
file_index = int(re.search('(.*?).npy', file).group(1))-1 
int(re.search('\\(.*?.csv',TRAIN_files[0]))
set(TRAIN_input[:,16])
#%%
TEST_files = sorted(glob.glob('D:/ie_201008/tets/*.csv'))
TEST_list = np.load('D:/ie_201008/train/test_list.npy')
TEST_input =np.load('D:/ie_201008/train/test_input.npy')
TRAIN_input[0,0]
TRAIN_list[0]
np.min(TRAIN_input[:,0])
